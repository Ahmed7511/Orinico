          ...
          if (cartItems && productsPanier) {
                ...
                <div class ="panier" >
                   <h5 class="name-product">${cartItem.name} </h5>  
                    ...
                    <h5 class="quantity-product"><i id="accre" class="far fa-plus-square"></i> ${
                              cartItem.quantity
                    } 
                     <i id="dec" class="fas fa-minus-circle"></i></h5>  
                    <h5 class ="total-product" >${
...
for (var i = 0; i < removeBtn.length; i++) {
          ...
          button.addEventListener("click", function (event) {
                    alert(
                              "vous pouvez retourner vers la page d'acceuil et choisir le produit que vous plaira !! "
                    );
          ...
          let responseData = await fetch(
                    ...
                    .then((data) => {
                              ...
                              localStorage.setItem(
                                        "orderRecap",
                                        JSON.stringify(orderRecap)
                              ); // on créé un localStorage qui contient le recap
